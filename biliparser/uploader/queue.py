...
class UploadQueueManager(ABC):
    ...
    async def cancel_user_tasks(self, user_id: int) -> int:
        async with self._lock:
            active = self.active_tasks.pop(user_id, {})
            processing = self.processing_tasks.pop(user_id, {})
            for process_task in processing.values():
                process_task.cancel()
            cancelled_count = len(set(active) | set(processing))
    ...
    async def _process_upload(self, task: UploadTask) -> None:
        max_retries = max(1, int(os.environ.get("UPLOAD_MAX_RETRIES", 4)))
    ...
    async def _retry_parse_url(self, task: UploadTask) -> bool:
        ...
        try:
            ...
            results = await self.registry.parse(
    ...
    async def _try_upload_once(self, task: UploadTask, attempt: int, max_retries: int) -> bool:
        ...
        try:
            # fetch 任务由 channel 自己按原文件/document 语义准备媒体；若先走通用下载，
            # 不仅会重复下载，还会在 Telegram 实现中对同一个 Redis key 嵌套加锁。
