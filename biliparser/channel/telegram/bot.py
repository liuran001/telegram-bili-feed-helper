...
from .uploader import TelegramUploadQueueManager, TelegramUploadTask, _send_kind, get_cached_media_file_id
...
def build_application(channel, provider_registry: ProviderRegistry, manage_db: bool = True) -> Application:
    ...
    application = (
        ...
        # 上传大体积视频容易超时，超时时间改为可配置；media_write_timeout 默认上调到 600s
        .media_write_timeout(int(os.environ.get("MEDIA_WRITE_TIMEOUT", 600)))
        .read_timeout(int(os.environ.get("READ_TIMEOUT", 60)))
        .write_timeout(int(os.environ.get("WRITE_TIMEOUT", 120)))
